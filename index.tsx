    ...
    <div className="min-h-screen bg-white pb-40 fade-in p-4 pt-8 text-left">
      ...
      {activeTab === 'settings' && (
        <div className="bg-white p-8 rounded-[40px] shadow-soft border space-y-8 text-left">
          <div className="space-y-4">
            <h3 className="text-xs font-black text-primary uppercase tracking-widest">Brand & Theme</h3>
            <div>
          ...
          <div className="space-y-4 pt-6 border-t">
            <h3 className="text-xs font-black text-primary uppercase tracking-widest">Shop Configuration</h3>
            ...
            <div><label className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-2 block ml-2">Delivery Fee (₹)</label><input type="number" className="w-full p-4 bg-gray-50 rounded-2xl outline-none font-bold" value={appData.settings.deliveryFee} onChange={e => onUpdateAppData({...appData, settings: {...appData.settings, deliveryFee: parseInt(e.target.value)||0}})} /></div>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 z-[100] bg-black/60 backdrop-blur-sm flex items-end justify-center p-4">
          <div className="bg-white w-full max-w-md rounded-t-[40px] p-8 animate-slideUp overflow-y-auto max-h-[90vh]">
            ...
            <div className="space-y-4 text-left">
              ...
              <div><label className="text-[10px] font-black text-gray-400 uppercase mb-1 ml-1 block">DESCRIPTION</label><textarea className="w-full p-4 bg-gray-50 rounded-xl border font-medium text-sm resize-none" rows={3} value={editingProduct.description} onChange={e => setEditingProduct({...editingProduct, description: e.target.value})} /></div>
...
const App = () => {
  ...
  useEffect(() => {
    const init = async () => {
      try {
        ...
        const savedData = localStorage.getItem('ramdev_app_data_v3');
        if (savedData) {
        ...
        const storedProfile = localStorage.getItem('ramdev_profile_v3');
        if (storedProfile) { setProfile(JSON.parse(storedProfile)); setView('catalog'); }
        const storedOrders = localStorage.getItem('ramdev_orders_v3');
  ...
  useEffect(() => {
    if (!isLoading) {
      localStorage.setItem('ramdev_app_data_v3', JSON.stringify(appData));
    }
  }, [appData, isLoading]);

  // Handle Dynamic Theming
  useEffect(() => {
  ...
  const handleOrderConfirm = () => {
    ...
    localStorage.setItem('ramdev_orders_v3', JSON.stringify(updatedOrders));
    ...
    <div className="min-h-screen bg-white p-10 flex flex-col items-center justify-center text-center fade-in">
      ...
      <div className="space-y-6 w-full max-w-xs text-left">
        ...
        <button onClick={() => {
          ...
          if (p.fullName && p.phone && p.address) { setProfile(p); localStorage.setItem('ramdev_profile_v3', JSON.stringify(p)); setView('catalog'); }
